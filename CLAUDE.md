...
- Source files live next to each CSS file as `.scss` (e.g. `style.scss` compiles to `style.css`).
- Edit `.scss` only. `.css` files are build artifacts regenerated by the file watcher (user runs locally).
- Use nested BEM: `.block { &__element {} &--modifier {} }`.
- Media queries stay inline inside the nested block where they apply.
- Commit BOTH `.scss` (source) and `.css` (compiled output) — so the theme works without a build step.
- Do NOT introduce a package manager / webpack / gulp. The file watcher is the only build tool.
